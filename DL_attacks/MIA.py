...
def mia_for_each_nn(dl, attacker, get_model):
    ...
    for i, v in enumerate(nn):
        ...
        assign_list_variables(model.trainable_variables, var) #! 将var赋值给model.trainable_variables
...
def MIA_received_model(
    ...
    get_model = lambda attacker, u: attacker.model_update_buffer[u.name] #! 比较所有真实的邻居
...
def MIA_funcIsolated_model(
    ...
    get_model = lambda attacker, u: attacker.get_functional_isolated_model(u.name) #! 比较所有被isolate出来的邻居
    
    return mia_for_each_nn(dl, attacker, get_model) #! 返回的是一个mia_for_each_nn的结果(一个数组，每个元素是一个邻居的MIA得分)
